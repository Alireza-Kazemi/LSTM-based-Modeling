function dataTask = PreGoTask(dataEEG,preTask,stims)
...
for subj = 1:length(dataEEG)
    ...
    for stimIdx = 1:length(stims)
        ...
        dataTask{stimIdx,subj} = zeros(size(eeg,1),preTask*fs,length(trials));
        for trialIdx = 1:length(trials)
            idxEnd   = trials(trialIdx)-1;
            idxStart = idxEnd - preTask*fs+1;
